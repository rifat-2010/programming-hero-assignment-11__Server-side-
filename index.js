...
async function run() {
  try {
    ...
    // data fetching from mongodb and https://book-courier-server-kappa.vercel.app/ server created
    // find
    // findOne
    ...
    app.post("/wishlist", async (req, res) => {
      try {
        ...
        if (!data.userId || !data.bookId) {
          return res
            .status(400)
            .send({ success: false, message: "userId and bookId required" });
        }
        ...
        if (exists) {
          return res
            .status(400)
            .send({ success: false, message: "Book already in wishlist" });
        }
    ...
    app.put("/books/status/:id", async (req, res) => {
      try {
        ...
        if (!status) {
          return res
            .status(400)
            .send({ success: false, message: "Status required" });
        }
    ...
    app.put("/users/role/:id", async (req, res) => {
      ...
      if (!role) {
        return res
          .status(400)
          .send({ success: false, message: "Role required" });
      }
